...
class Deck(collections.Counter[cards.Card]):
    ...
    @property
    def crypt(self) -> list[tuple[cards.Card, int]]:
        """For convenience, list of crypt (card, count)."""
        return list(self.cards(lambda c: c.crypt))

    @property
    def library(self) -> list[tuple[cards.Card, int]]:
    ...
    def _sorted_library(self) -> Generator:
        ...
        def _type_index(card_count: tuple[cards.Card, int]) -> int:
