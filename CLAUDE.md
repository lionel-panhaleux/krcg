...
- **Comments explain the unexpected, not the code.** Let the code speak for itself; comment only external constraints or the non-obvious reasoning behind otherwise-puzzling implementations. Public classes and functions still get a Google-style docstring (lint requires it — see above), but keep it to the minimum that serves the user.
